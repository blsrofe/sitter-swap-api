...
const account = (request, response, next) => {
    ...
    let idToken = request.header("Authorization")
    User.hasAccount(idToken)
    .then((data) => {
      if (!data) { return response.send('no account') }
      response.json(data)
    })
    .catch((err) => console.error(err))
}
...
module.exports = {getUser, signUp, signIn, getCurrentRequests, getPublic, logout, account}

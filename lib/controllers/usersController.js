...
const getCompletedRequests = (request, response, next) => {
    let id = request.params.id
    User.getCompleted(id)
    .then((data) => {
      if (!data) { return response.sendStatus(404) }
      response.json(data)
    })
...
module.exports = {getUser, signUp, signIn, getCurrentRequests, getPublic, logout, account, getAcceptedRequests, getCompletedRequests, updatePaws}

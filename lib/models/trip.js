...
const getActiveTrips = () => {
  return database.raw("SELECT trips.trip_name, trips.num_nights, trips.notes, trips.start_date, trips.end_date, users.username FROM trips WHERE status=? INNER JOIN users ON trips.user_id=users.id", ["active"])
  .then((data) => {
    return data.rows
  })
}

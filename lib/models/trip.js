...
const getActiveTrips = () => {
  return database.raw("SELECT trips.trip_name, trips.num_nights, trips.notes, trips.start_date, trips.end_date, users.cross_street1, users.cross_street2, users.zip, trips.user_id FROM trips INNER JOIN users ON trips.user_id=users.id WHERE trips.status=?", ["active"])
  .then((data) => {
    return data.rows
  })
}

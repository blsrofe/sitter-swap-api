...
const postTrip = (tripInfo) => {
  return database.raw('INSERT INTO trips (trip_name, user_id, start_date, end_date, num_nights, notes, status, accepter_id, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?) RETURNING id, trip_name, user_id, num_nights', [tripInfo.tripName, tripInfo.userId, tripInfo.startDate, tripInfo.endDate, tripInfo.numNights, tripInfo.notes, "active", "none", new Date])
  .then((data) => {
    return data.rows[0]
  })
}
...
const getOwner = (id) => {
  return database.raw("SELECT user_id FROM trips WHERE id=?", [id])
  .then((data) => {
    return data.rows
  })
}

...
const getActiveTrips = () => {
  return database.raw("SELECT trips.id, trips.trip_name, trips.num_nights, trips.notes, trips.start_date, trips.end_date, trips.status, trips.users.cross_street1, users.cross_street2, users.zip, users.first_name, trips.user_id FROM trips INNER JOIN users ON trips.user_id=users.id WHERE trips.status=?", ["active"])
  .then((data) => {
    return data.rows
  })
}

...
const getResponses = (id) => {
  return database.raw("SELECT messages.message, messages.sender_id, users.first_name FROM messages INNER JOIN users ON messages.sender_id=users.id WHERE trips.id=? AND messages.response_to_request=?", [id, "true"])
  .then((data) => {
    return data.rows
  })
}
